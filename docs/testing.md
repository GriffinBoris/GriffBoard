...
- Type `teh` followed by Space and confirm `the` replaces it. Tap the Undo chip or immediately press Backspace to restore `teh`. Check that cursor moves, field changes, and keyboard closure remove Undo, and that disabling auto-correct leaves the typo unchanged.
...
- v0.3.0 release preparation: twenty-four JVM tests and all sixteen API 36 ARM64 emulator tests passed, including Tiny English transcription and microphone stop/cancellation. Debug/release lint, signed release packaging, release-script checks, guidance validation, and package-integrity checks passed. Visually checked the shorter, smoothed waveform using synthetic sample peaks and the adjusted numeral text size. Physical-device performance remains unverified.
- Dictionary correction and transcription pulse: twenty-six JVM tests and twelve focused emulator tests passed. Covered single-edit correction, frequency ambiguity, valid-word preservation, case/identifier exclusions, and dismissal/undo with the bundled dictionary. Confirmed `keyboarf` previews `keyboard` and Space accepts it in the actual IME. Verified visible waveform animation during transcription and restoration when transcription ends. Debug/release lint, builds, release-script checks, and guidance checks passed. Recording-waveform previews use synthetic peaks; physical-phone behavior still needs verification.
- Toolbar spacing and auto-correct: twenty-three JVM tests and eight focused emulator tests passed, plus debug/release lint and builds. Tested conservative typo rules, case and identifier exclusions, correction/undo with punctuation and surrounding text, selection and mid-word exclusions, stale cursor/context rejection, partial extracted-text offsets, and the tappable Undo chip. Manually confirmed Space acceptance, chip undo, and immediate Backspace undo in the actual IME. Checked smaller toolbar visuals and 12 dp of additional bottom separation on the emulator; Samsung navigation spacing still needs a physical-device check.
- Correction preview: twenty-three JVM tests and ten focused emulator tests passed. The preview has separate Accept and Dismiss controls; dismissal preserves that word occurrence without suppressing later corrections. Verified Space acceptance, X dismissal, and Enter acceptance in the actual IME. Toolbar backgrounds use the same 4 dp horizontal gaps as the keyboard keys, with a separately padded X control. Debug/release lint and builds passed.
- Number keys and waveform bars: twenty-four JVM tests and ten focused emulator tests passed, along with `task local:check`, release lint, and the signed release build. Visually checked the suggestion-only preview and matching number/letter key dimensions in the actual IME. The waveform groups real signed sample peaks into rounded bars with gaps; its visual check used synthetic samples. Empty, silent, and below-threshold audio bypass transcription, and whitespace-only transcripts never reach text insertion. The preview APK passed signature and 16 KB ZIP alignment checks.
