...
- Auto-correct previews explicit common-typo rules and accepts them at Space, punctuation, or Enter unless dismissed. Dismissal belongs only to that word occurrence and editor context. Keep undo in the current input session, validate the absolute cursor and surrounding text before replacement, and clear it on new edits, field changes, or keyboard closure. Do not promote fuzzy suggestions into automatic replacements.
